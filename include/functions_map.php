...
function swisstopocopyright($attrleaflet, $attrimagery, $attrmodule, $bl, $custombaselayer)
{
    $return = "";

    if ($attrleaflet) $return .= '<a href="http://leafletjs.com/" target="_blank">Leaflet</a> ';

    if ($attrmodule) $return .= l10n('PLUGINBY').' <a href="https://github.com/xbgmsharp/piwigo_swisstopo" target="_blank">xbgmsharp</a> ';

    if ($attrimagery)
    {
        $return .= " ";
        if     ($bl == 'swisstopo')	$return .= "Swisstopo";
    }
    // Mandatory by http://www.swisstopo.ch

    $return .= ' &copy; ';
    $return .= l10n("SWISSTOPO_CONTRIBUTORS");
    return $return;
}

function swisstopo_get_gps($page)
{
    ...
    if (isset($page['section']))
    {
        ...
        {
            $items = get_image_ids_for_tags( array_reduce( $page['tags'], 'swisstopo_get_page_tag_id' ) );
            if ( !empty($items) )
            {
...
function swisstopo_get_items($page)
{
    // Limit search by category, by tag, by smartalbum
    $LIMIT_SEARCH="";
    $INNER_JOIN="";
    ...
    if (isset($_GET['min_lat']) and isset($_GET['max_lat']) and isset($_GET['min_lng']) and isset($_GET['max_lng']))
    {
        ...
        $query="SELECT `id` FROM ".CATEGORIES_TABLE." WHERE `name` = 'Locations' AND `comment` LIKE '%SWISSTOPO plugin%';";
        ...
        /* Create an album */
        $options = array(
            'comment'=> 'Generated by SWISSTOPO plugin',
        );
        $swisstopo_album = create_virtual_category('Locations', NULL, $options);

        /* Create a sub album */
        $options = array(
            'comment'=> "SWISSTOPO virtual album\nlat:".$_GET['min_lat']." ".$_GET['max_lat']."\nlng:".$_GET['min_lng']." ".$_GET['max_lng'],
        );
        $swisstopo_sub_album = create_virtual_category("SWISSTOPO".$_GET['min_lat']."", $swisstopo_album['id'], $options);
    ...
    ".$forbidden.";";
        ...
        {
            $query="INSERT INTO ".IMAGE_CATEGORY_TABLE." ( `image_id` ,`category_id` ,`rank` ) VALUES ( '".$item['id']."', '".$swisstopo_sub_album['id']."', NULL );";
            pwg_query($query);
        }

        /* Redirect to the new album */
        header('Location: '.get_absolute_root_url().'index.php?/category/'.$swisstopo_sub_album['id']);
        exit;
    }
...
function swisstopo_get_js($conf, $local_conf, $js_data)
{
    // Load parameter, fallback to default if unset
    if (isset($local_conf['popup']))
        $popup = $local_conf['popup'];
    else
        $popup = isset($conf['swisstopo_conf']['left_menu']['popup']) ? $conf['swisstopo_conf']['left_menu']['popup'] : 0;
    $linkname = isset($conf['swisstopo_conf']['left_menu']['link']) ? $conf['swisstopo_conf']['left_menu']['link'] : 'OS World Map';
    $popupinfo_name = isset($conf['swisstopo_conf']['left_menu']['popupinfo_name']) ? $conf['swisstopo_conf']['left_menu']['popupinfo_name'] : 0;
    $popupinfo_img = isset($conf['swisstopo_conf']['left_menu']['popupinfo_img']) ? $conf['swisstopo_conf']['left_menu']['popupinfo_img'] : 0;
    $popupinfo_link = isset($conf['swisstopo_conf']['left_menu']['popupinfo_link']) ? $conf['swisstopo_conf']['left_menu']['popupinfo_link'] : 0;
    $popupinfo_comment = isset($conf['swisstopo_conf']['left_menu']['popupinfo_comment']) ? $conf['swisstopo_conf']['left_menu']['popupinfo_comment'] : 0;
    $popupinfo_author  = isset($conf['swisstopo_conf']['left_menu']['popupinfo_author']) ? $conf['swisstopo_conf']['left_menu']['popupinfo_author'] : 0;
    $baselayer = isset($conf['swisstopo_conf']['map']['baselayer']) ? $conf['swisstopo_conf']['map']['baselayer'] : 'mapnik';
    ...
    $pinid = isset($conf['swisstopo_conf']['pin']['pin']) ? $conf['swisstopo_conf']['pin']['pin'] : 1;
    $pinpath = isset($conf['swisstopo_conf']['pin']['pinpath']) ? $conf['swisstopo_conf']['pin']['pinpath'] : '';
    $pinsize = isset($conf['swisstopo_conf']['pin']['pinsize']) ? $conf['swisstopo_conf']['pin']['pinsize'] : '';
    $pinshadowpath = isset($conf['swisstopo_conf']['pin']['pinshadowpath']) ? $conf['swisstopo_conf']['pin']['pinshadowpath'] : '';
    $pinshadowsize = isset($conf['swisstopo_conf']['pin']['pinshadowsize']) ? $conf['swisstopo_conf']['pin']['pinshadowsize'] : '';
    $pinoffset = isset($conf['swisstopo_conf']['pin']['pinoffset']) ? $conf['swisstopo_conf']['pin']['pinoffset'] : '';
    $pinpopupoffset = isset($conf['swisstopo_conf']['pin']['pinpopupoffset']) ? $conf['swisstopo_conf']['pin']['pinpopupoffset'] : '';
    ...
    // Load baselayerURL
    if     ($baselayer == 'swisstopo')     $baselayerurl = 'https://wmts20.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-farbe/default/current/3857/{z}/{x}/{y}.jpeg';

    $attribution = swisstopocopyright($attrleaflet, $attrimagery, $attrmodule, $baselayer, $custombaselayer);
    ...
    $editor = isset($local_conf['editor']) ? "editInSWISSTOPOControlOptions: { editors: ['id'] }, " : '';
    ...
    if ($pinid >= 2)
    {
        ...
        var PlgIcon = L.Icon.extend({
            options: {
                shadowUrl: 'plugins/piwigo_swisstopo/leaflet/images/marker-shadow.png',
                iconSize:     [25, 41],
                shadowSize:   [41, 41],
                iconAnchor:   [21, 21],
                popupAnchor:  [-10, -10]
        ...
        var LeafIcon = L.Icon.extend({
            options: {
                shadowUrl: 'plugins/piwigo_swisstopo/leaflet/images/leaf-shadow.png',
                iconSize:     [38, 95],
                shadowSize:   [50, 64],
                iconAnchor:   [22, 94],
                shadowAnchor: [4, 62],
        ...
        var MapIcon = L.Icon.extend({
            options: {
                shadowUrl: 'plugins/piwigo_swisstopo/leaflet/images/mapicons-shadow.png',
                iconSize:     [32, 37],
                shadowSize:   [51, 37],
                iconAnchor:   [19, 38],
                shadowAnchor: [4, 33],
        ...
        var PlgIconGreen = new PlgIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/marker-green.png'}),
            PlgIconRed = new PlgIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/marker-red.png'});

        var LeafIconGreen = new LeafIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/leaf-green.png'}),
            LeafIconRed = new LeafIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/leaf-red.png'}),
            LeafIconOrange = new LeafIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/leaf-orange.png'});

        var MapIconBlue = new MapIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/mapicons-blue.png'}),
            MapIconGreen = new MapIcon({iconUrl: 'plugins/piwigo_swisstopo/leaflet/images/mapicons-green.png'});
...
function swisstopo_gen_template($conf, $js, $js_data, $tmpl, $template)
{
    ...
    $template->assign(
        array(
            ...
            'default_baselayer' => $conf['swisstopo_conf']['map']['baselayer'],
        )
    );

    if ( $conf['swisstopo_conf']['map']['baselayer'] == 'custom' ) {
        $iconbaselayer = $conf['swisstopo_conf']['map']['custombaselayerurl'];
        ...
        $template->assign(
            array(
                'custombaselayer'    => $conf['swisstopo_conf']['map']['custombaselayer'],
                'custombaselayerurl' => $conf['swisstopo_conf']['map']['custombaselayerurl'],
...
function swisstopo_parse_map_data_url($tokens, &$next_token)
{
    ...
    $page['start']=0;
    $page['box'] = swisstopo_bounds_from_url( @$_GET['box'] );
    return $page;
}

function swisstopo_bounds_from_url($str)
{
 ...
 * @return basket
 */
function swisstopo_get_page_tag_id($basket, $page_tag) {
	$basket[] = $page_tag['id']; 
	return $basket;
}
